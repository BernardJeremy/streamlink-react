...
module.exports.getDirectLink = (socket, req) => {
  const parsedUrl = urlParser.parse(req.url);

  if (!parsedUrl) {
    socket.emit('server.GetDirectLink', { err: true, html: "Twitch URL not recognized or incomplete" });
    return;
  }
  ...
  getTwitchLinkService(req.url, req.token).then((html) => {
    ...
    socket.emit('server.GetDirectLink', { err: true, html: possibleFailText });
  });
};
